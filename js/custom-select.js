    ...
    if (Array.isArray(currDropOptions)) {
      ...
      $.each(currDropOptions, (i, el) => {
        currentDropdown.parent().siblings('.fancy-dropdown-options').children('.options-list').append(
          `<li class="option-item" data-value="${el}">${el}</li>`)
      });
    } else {
...
const handleOptionSelect = () => {
  ...
  $(document).on("click", ".fancy-dropdown-options .option-item", (e) => {
    ...
    optionItem.parents('.fancy-dropdown-wrapper').find('.fancy-dropdown').trigger('customChange', { 'value': optionValue, 'text': optionText})
  });
}
...
const setInitialValues = () => {
  $('.fancy-dropdown').each((i, el) => {
    ...
    const input = $(el);
    const optionItems = options.find(".option-item");
    for(let optionItemIndex = 0; optionItemIndex < optionItems.length; optionItemIndex ++) {
        if($(optionItems[optionItemIndex]).attr('data-value') == input.val()) {
            text = $(optionItems[optionItemIndex]).text();
            value = $(optionItems[optionItemIndex]).attr('data-value');
        }
    }
